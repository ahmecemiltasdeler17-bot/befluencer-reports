...
export function TikTokContentCard({
  ...
  return (
    <article className="report-content-card pdf-avoid-break overflow-hidden rounded-2xl bg-[var(--report-surface)] ring-1 ring-inset ring-[var(--report-border)]">
      {showCreatorHeader && (
        <ReportCreatorLink
          link={creatorLink}
          className="flex items-center gap-3 border-b border-[var(--report-border)] px-4 py-3"
        >
          <SafeAvatar
          ...
          <div className="min-w-0 flex-1">
            <p className="flex items-center gap-1.5 text-sm font-medium text-[var(--report-text)]">
              <span className="truncate">{video.creatorHandle}</span>
              {creatorLink && <ReportExternalLinkIcon />}
            </p>
            <p className="truncate text-xs text-[var(--report-text-tertiary)]">
      ...
      <div className="relative aspect-[9/16] w-full overflow-hidden bg-[var(--report-surface-elevated)]">
        <div className="report-content-card__poster absolute inset-0">
        ...
        <div className="absolute top-3 right-3">
          <div className="report-content-card__play flex size-8 items-center justify-center rounded-full bg-black/50 text-white ring-1 ring-[var(--report-accent)]/35 backdrop-blur-sm">
            <Play className="size-3.5 fill-current" />
          </div>
        </div>

        {/* Sits under the play chip: the fallback poster already fills the
            bottom edge with handle/caption text. */}
        {videoLink ? (
          <span className="report-content-card__watch screen-only absolute top-[3.1rem] right-3 rounded-full bg-[var(--report-accent)]/90 px-2 py-0.5 text-[9px] font-semibold tracking-[0.1em] text-[var(--report-bg)] uppercase print:hidden">
            İzle
          </span>
        ) : null}
      ...
      <div className="space-y-3 px-4 py-4">
        {video.title ? (
          <p className="line-clamp-2 text-sm leading-snug text-[var(--report-text-secondary)]">
            {video.title}
          </p>
        ) : null}
        {video.hasMetrics === false ? (
          <p className="text-sm text-[var(--report-text-tertiary)]">Henüz metrik yok</p>
        ) : (
          <>
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-1.5 text-sm font-semibold text-[var(--report-text)] tabular-nums">
                <Eye className="size-3.5 text-[var(--report-text-tertiary)]" aria-hidden />
            ...
            <div className="grid grid-cols-4 gap-2 border-t border-[var(--report-border)] pt-3">
...
function MiniMetric({
  ...
  return (
    <div className="text-center" title={label}>
      <Icon className="mx-auto size-3.5 text-[var(--report-text-tertiary)]" />
      <p className="mt-1 text-[11px] font-medium text-[var(--report-text-secondary)] tabular-nums">
        {value}
      </p>
    </div>
  );
